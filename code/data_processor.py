...
def _extract_images(lists):
    for line in lists:
        temp = line.split('/')
        # path1= os.path.join(os.path.join(args.root_path , 'image'),temp[-3])
        path1 = os.path.join(args.root_path , 'image')
        if not os.path.exists(path1):
            os.mkdir(path1)
        path2 = os.path.join(path1, temp[-2])
        if not os.path.exists(path2):
            os.mkdir(path2)
        path3 =  os.path.join(path2, temp[-1][:-4])
        if not os.path.exists(path3):
            os.mkdir(path3)

        path_img = line.replace('video','image').replace('.mpg','/') + temp[-1][:-4]
        # print(path1,path2, path3, path_img)
        command = ' ffmpeg -i ' + line + ' -r 25 ' + path_img + '_%5d.jpg'
        # print(command)
        try:
            # pass
            os.system(command)
...
def extract_images():
    ...
    img = os.path.join(args.root_path ,  'image')
...
def _extract_audio(lists):
    for line in lists:
        temp = line.split('/')
        path1 = os.path.join(args.root_path , 'audio')
        if not os.path.exists(path1):
            os.mkdir(path1)
        path2 = os.path.join(path1, temp[-2])
        if not os.path.exists(path2):
        ...
        command = 'ffmpeg -i ' + line + ' -ar 44100  -ac 1  ' + line.replace('video','audio').replace('.mpg','.wav')
        try:
            # pass
            os.system(command)
        except BaseException:
...
def get_base(image_path):
    ...
    np.save('../basics/base_male.npy', preds)
...
# crop_image('../image/test.jpg')
# get_base( '../basics/base_male.jpg') # png')
...
def generating_landmark_lips(lists):
    ...
    for line in image_txt:
        try:
            ...
            img_path = line
            temp = img_path.split('/')
            face_path = os.path.join(args.root_path,'faces') + '/' + temp[-4]
            face_path_1 = os.path.join(face_path, temp[-3])
            face_path_2 = os.path.join(face_path, temp[-2])
            lmark_path = os.path.join(args.root_path,'landmark3d') + '/' + temp[-4]
            lmark_path_1 = os.path.join(lmark_path, temp[-3])
            lmark_path_2 = os.path.join(lmark_path, temp[-2])
            ...
            if not os.path.exists(face_path_1):
                os.mkdir(face_path_1)

            if not os.path.exists(lmark_path_1):
                os.mkdir(lmark_path_1)

            if not os.path.exists(face_path_2):
                os.mkdir(face_path_2)

            if not os.path.exists(lmark_path_2):
                os.mkdir(lmark_path_2)

            landmark_path = lmark_path_2 + '/' + temp[-1][:-4] + '.npy'
            lip_path = face_path_2 + '/' + temp[-1][:-4] + '.jpg'
...
def __visualization(img_path, fake_lmark):
    ...
    temp = img_path.split('/')
    vis_path = os.path.join('../visualization', temp[-4])
    vis_path_1 = os.path.join(vis_path, temp[-3])
    vis_path_2 = os.path.join(vis_path, temp[-2])
    if not os.path.exists( vis_path ):
        os.mkdir( vis_path )
    if not os.path.exists( vis_path_1 ):
        os.mkdir( vis_path_1)
    if not os.path.exists( vis_path_2 ):
        os.mkdir( vis_path_2 )
...
def __visualization2d(img_path, fake_lmark):
    ...
    landmark_path = os.path.join('./','landmark1d') + '/' + \
        temp[-4] + '/' + temp[-3] + '/' + temp[-2] + '/' + temp[-1][:-4] + '.npy'
...
def visualization():
    _file = open(os.path.join('./pickle/', "new_img_full_gt_test.pkl"), "rb")
...
def data_gt_prepare():
    ...
    #         if not os.path.exists('../dataset/landmark1d/' + lmark_path):
    #                 continue
    ...
    #             ggg = np.load('../dataset/landmark1d/'+ lmark_path)
    ...
    # shuffle(data[0])
    # shuffle(data[1])
    # with open(os.path.join('../datadset/','pickle/img_region_gt_warp_train.pkl'), 'wb') as handle:
    #     pickle.dump(data[0], handle, protocol=pickle.HIGHEST_PROTOCOL)
    # with open(os.path.join('../dataset/','pickle/img_region_gt_warp_test.pkl'), 'wb') as handle:
    #     pickle.dump(data[1], handle, protocol=pickle.HIGHEST_PROTOCOL)


def data_lstm_gt_prepare():
    train_set = []
    test_set = []
    count = 0
    root_path = '../dataet/landmark1d/'
    words =  os.listdir(root_path)
    words  = sorted(words)
    for word in words:
        print(word)
    ...
    # with open(os.path.join('../dataset/' + 'pickle/region_16_warp_gt_train2.pkl'), 'wb') as handle:
    #     pickle.dump(data[0], handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open(os.path.join('../dataset/','pickle/region_16_warp_gt_test2.pkl'), 'wb') as handle:
        pickle.dump(data[1], handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def audio2mfcc():
    ...
    for word in os.listdir(root_path):
        if not os.path.exists( os.path.join('../mfcc', word) ):
            os.mkdir( os.path.join('../mfcc', word))
        ...
        status = ['train','val','test']
        for s in status:
            if not os.path.exists( os.path.join('../mfcc', word, s) ):
                os.mkdir( os.path.join('../mfcc', word, s))
            for name in  os.listdir( os.path.join(root_path, word, s)):
                ...
                if name[-3:] == 'wav':
                    ...
                    mfcc_name = os.path.join('../mfcc', word, s, name[:-3] + 'npy')
...
def data_demo_prepare():
    train_set = []
    count = 0
    demo_set  = []
    root_path = '../demo/regions'
    for word in os.listdir(root_path):
        count += 1
        if count > 50:
            break
    ...
    print (demo_set)
    with open(os.path.join('./','pickle/new_img_full_gt_demo.pkl'), 'wb') as handle:
        pickle.dump(demo_set, handle, protocol=pickle.HIGHEST_PROTOCOL)
def data_lstm_demo_prepare():
    train_set = []
    count = 0
    demo_set  = []
    root_path = '../demo/regions'
    ...
    print (len(demo_set[0]))
    print (demo_set)
    with open(os.path.join('./','pickle/new_16_full_gt_demo.pkl'), 'wb') as handle:
        pickle.dump(demo_set, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def pca_lmark_lrw():
    k = 20
    lmark_root_path = '../dataset/landmark1d/'
    _file = open( './pickle/region_16_warp_gt_test2.pkl', "rb")
...
def test_pca():
    ...
    _file = open( '../dataset/pickle/region_16_warp_gt_train2.pkl', "rb")
    train_data = pickle.load(_file)
    # shuffle(train_data)
    ...
    for i in  range(10):
        ...
        lmark_path = os.path.join('../dataset/landmark1d' ,train_data[index][0][:-8] + '.npy')
        t_lmark = np.load(lmark_path)

        a = torch.from_numpy(t_lmark).view(29,136)
        for jj in range(29):
            path = os.path.join( '../dataset/regions/',  train_data[index][0][:-8] + '_%03d.png'%(jj+1) )
            b = a[jj:jj+1]
...
def data_lstm_lmark_prepare():
    ...
    for word in os.listdir(root_path):
        ...
        for s in status:
            for video in  os.listdir( os.path.join(root_path, word, s)):
                ...
                mfcc_path = os.path.join('../dataset/mfcc/' , word, s, video + '.npy')
    ...
    with open(os.path.join('../dataset','pickle/lmark_train.pkl'), 'wb') as handle:
        pickle.dump(train_set, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open(os.path.join('../dataset','pickle/lmark_test.pkl'), 'wb') as handle:
