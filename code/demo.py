...
def generator_demo_example_lips(img_path):
    ...
    landmark_path = os.path.join('../temp/', name.replace('jpg', 'npy')) 
    region_path = os.path.join('../temp/', name.replace('.jpg', '_region.jpg')) 
...
def test():
    ...
    with torch.no_grad(): 
        ...
        np.save( os.path.join( config.sample_dir,  'fake.npy'), fake_lmark)
        ...
        print ('The generated video is: {}'.format(os.path.join(config.sample_dir , 'results-all.mp4')))
